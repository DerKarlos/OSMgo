...
export class WedgeGeometry extends THREE.BufferGeometry {
	...
	constructor(shape = new THREE.Shape([new THREE.Vector2(0.5, 0.5), new THREE.Vector2(- 0.5, 0.5), new THREE.Vector2(- 0.5, - 0.5), new THREE.Vector2(0.5, - 0.5)]), options = {}) {
		...
		this.setAttribute('uv', new THREE.BufferAttribute(new Float32Array([1., 1.]), 2)); // ++karlos++

		this.computeVertexNormals();

		// var roof_positions = positions.length
		this.groups.push({ start: /*........*/ 0, count: roof_positions, materialIndex: 0 })
		this.groups.push({ start: roof_positions, count: positions.length, materialIndex: 1 })
	}
	...
	*/
	splitShape(points) {
		...
		// TEST: https://osmgo.org/v03.html?km=1&lat=49.58298499&lon=11.01612998&ele=40.02&dir=4&view=-31&user=schleswig-holstein&dbg=0&con=1&tiles=5&opt=2
		...
		var point = [];
		var nextPoint = [];
		var prevPoint = points[points.length - 2];  // - 2 because the last is = the first
