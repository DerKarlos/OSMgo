...
Integrator.prototype.update = function(active,plus,minus,aUp,aMax,dt,aDown) { with(this) {
	...
	if(dt>33) dt= 33 // schummel aber beruhigt Bewegungen
	if(!aDown) aDown = 0
...
function goControl(webGLRenderer,camera) {
	...
	// 1=Inspect =?OSMBuildings   2=SegWay   3=...
...
this.Reset = function(home) { with(this) {
	keyAlt = keyShilft = keyUp = keyDown = keyPUp = keyPDown = keyRight = keyLeft = keyEnter = keySpace = false
	vCardF = 0
...
this.update = function(dt) { with(this) {
	...
	if( dt>0.2) {	// Wenn Javascript mal wieder ein Päuschen macht,
		dt=0.2		//  soll das riesen dt nicht alles wild toben lassen. Also alles begrenzen:
		keyAlt = keyShilft = false // keyUp = keyDown = keyRight = keyLeft = keyEnter = keySpace = false
	}
	...
	if(DevOrConsOn) {
		...
		var aa = DOcontrols.alpha	// 0-360 wobei der Wert immer um 0 schwanken sollte
		if(aa>g(180)) aa -= g(360)  // aus 350 werden -10
		var ax = Math.abs(aa)-g(30)	// Zu viel verdreht
		if(ax>0) {
	    //	DOcontrols.alphaOffsetAngle -= ax * Math.sign(aa) * dt * 0.1 // Segway langsam nachdrehen
			alert(DOcontrols.alpha+" / "+DOcontrols.alphaOffsetAngle+" / "+aa)
		}
	}
	if(controlMode==3) {
		b = camera.rotation.x
	...
	if(b!=0) {		// View ahead up/down
		/*	Wie reagiert ein Segway? 
		    ...
		    vSoll   = Math.min(b*b*2000,100)  * Math.sign(b)	;if(vSoll>0) vSoll/=10 // backward solwer
...
/*		var rMax = 5
	...
	/*
	Verschiedene Modi: 1st-Person, OSMBuildings
	...
	OSMBuildings:		Tastatur					Maus & Wheel		Touch						Kardboard
	 no key    Spin:	+ XY, Z:on					+ XY, Z mit Wheel?	1touch+ XY, Z entfällt		Orientation-Control
	 keyShilft Shilft:	+ XY, Z mit #Enter/Space	+ XY, Z mit Wheel!	2touch+ XY, Z mit Abstand 	? Auto-Z? SegWay? flieger? quadrocopter?

	1st-Person:			Tastatur					Maus & Wheel		Touch						Kardboard
	no key		Walk    -Yspin  |Zshift  #Xspin?	wie Key				Wie OSMB?					
	keyShilft   Fly     -Xshift |Yshift  Zspin:no					
	...
	*/
	...
	var AltShift = keyAlt || keyShift



	if((this.controlMode==1) && (!DevOrConsOn)) { // 1=Inspect =?OSMBuildings   2=SegWay   3=...
	...
	if((this.controlMode==2) && (!DevOrConsOn)) { // 1=Inspect =?OSMBuildings   2=SegWay   3=...  sss sss sss
		// Position aus FR wird XZ. Dazu Y  (Forward,Right - X=Quer Z=Vorrück  Y=Höhe)
	...
	if(this.controlMode==1) { // 1=Inspect =?OSMBuildings   2=SegWay   3=...		
	...
	if(this.controlMode==2) { // 1=Inspect =?OSMBuildings   2=SegWay   3=...		
	...
	if( key_X) {
		key_X = false
		controlMode = (controlMode==1) ? 2:1
		switch(controlMode) {	//                                  slide on ground  dddd
...
/****** /
	// Und auch X-Z-räumlich begrenzen
	var x = 500
	if(((camera.position.x <  -x  ) 
	  ...
	  ||(camera.position.z >  +x  ) 
		...
		max_xz = true
   		var text = prompt("Gone far or troulbe with the controls?","")
		if(text) {
			log("FAR!:"+text)
	    	var xchg = new XMLHttpRequest() 
			    xchg.open("GET", "./xchg.php?chat="+userName+"!"+text+";#######", true)
				xchg.send()
		}
		Reset() // stop
	 } 
/**/
	...
	if( (camera.position.y<=0) && (lastCamY>0) ) {
...
//	light.target.position.set( camera.position )
  ...
  //  cameraShadow.position.set( camera.position.x-lightX,      0, camera.position.z-lightZ ) // ??? tut nichts!!!
	  ...
	  || key_M
		...
		/*if(!DevOrConsOn)??*/ {
			...
			//	var url = myURL+"/OSMgoSlippy.html?lat="+lat.toFixed(5)+"&lon="+lon.toFixed(5)
				var url = myURL+"/map?go=1&lat="+lat.toFixed(5)+"&lon="+lon.toFixed(5) + "&user="+userName
			...
			if(key_M && keyShift) {  // Reload default or with shift only 0
				...
				var url = myURL+"/index.html"       +  "?lat="+lat.toFixed(5) + "&lon=" + lon.toFixed(5) + "&ele="+camera.position.y.toFixed(2)
	...
	if(avatar != null) {
		var sc = SinCos(camera.rotation.y,0.5)
		avatar.rotation.y =  camera.rotation.y + g(90)  // -z?
		avatar.position.set( camera.position.x - sc[0], 0.5,
							 camera.position.z - sc[1] )
	}
	...
	if( DOcontrols.alphaOffsetAngle==0 && heading!=0) {
...
this.onMouseDown  = function(event) { with(this) {
	...
	PointHStart = PointH = event.pageX; PointDH = 0
	PointVStart = PointV = event.pageY; PointDV = 0
	PointStart()
...
this.onKeyUp   = function(event) { this.onKeyX(event.keyCode,false)                                     ;if(dbg>2) log("Key-:" + event.keyCode) }
...
this.onTouchStart = function(event) { with(this) {  /// Touch START ////////////////////
	...
	case 1: PointF = true;
			// Anfang der Bedienung merken
			PointHStart = PointH = event.touches[ 0 ].pageX; PointDH = 0
			PointVStart = PointV = event.touches[ 0 ].pageY; PointDV = 0
			PointStart() ;log("Point F:",PointH,PointV)
		    break; 
	case 2: PointS = true;
	        // Anfang der Bedienung merken: Mittelwert aus 1. und 2. Touch
			PointHStart = PointH = (event.touches[0].pageX+event.touches[1].pageX)/2; PointDH = 0
			PointVStart = PointV = (event.touches[0].pageY+event.touches[1].pageY)/2; PointDV = 0
...
THREE.DeviceOrientationControls = function( camera ) {
	...
	this.alphaStart = 0 // weg?undefined
	...
	this.update = function() {
		...
		var alpha  = scope.deviceOrientation.alpha ? g( scope.deviceOrientation.alpha ) + this.alphaOffsetAngle : undefined; // Z
        ...
        this.heading = scope.deviceOrientation.webkitCompassHeading ? scope.deviceOrientation.webkitCompassHeading : 0;
		...
		if(alpha != undefined) {
			this.alpha = alpha;
			// if(!this.alphaStart) this.alphaStart = alpha
			//alert("### "+alpha+"/"+this.alphaOffsetAngle)
		}
	};
