...
// "wedge": (gabled) https://github.com/Beakerboy/Threejs-Geometries/blob/main/src/WedgeGeometry.js

/**
 * Modify ExtrudeGeometry such that z varies with x and y . (as a wedge / gabled roof)
 */
export class WedgeGeometry extends THREE.BufferGeometry {
  constructor(
    ...
    // geometry.groups.push({ start: 0, count: positions, materialIndex: 0 })
    var roof_positions = positions.length / 3;
    ...
    var tba = new THREE.BufferAttribute(new Float32Array(positions), 3);
    this.setAttribute("position", tba); // setAttribute addAttribute
    this.setAttribute(
      "uv",
    ...
    // var roof_positions = positions.length
    this.groups.push({
      start: /*........*/ 0,
      count: roof_positions,
      materialIndex: 0,
    });
    this.groups.push({
      start: roof_positions,
  ...
  splitShape(points) {
    // ALERNATING CODES:
    // https://github.com/bjornharrtell/jsts <== https://gis.stackexchange.com/questions/216977/how-to-split-the-polygon-operation-using-of-jsts
    // https://alexbol99.github.io/flatten-js/Polygon.html#splitToIslands
    // TEST: https://osmgo.org/v03.html?km=1&lat=49.58298499&lon=11.01612998&ele=40.02&dir=4&view=-31
    // http://localhost:5173/index.html?km=1&lat=49.58298499&lon=11.01612998&ele=40.02&dir=4&view=-31
    // https://beakerboy.github.io/OSMBuilding/index.html?id=31361386
    // bad: https://www.openstreetmap.org/way/31361386
    // gut: https://www.openstreetmap.org/way/31361389
    // https://github.com/Beakerboy/OSMBuilding/issues/72#issuecomment-2781745075
    ...
    const newOutline = new THREE.Shape();

    // Walk the shape and find all crossings.
    var point = [];
    var nextPoint = [];
    var prevPoint = points[points.length - 1]; // - 2 because the last is = the first
    ...
    var currentShape = new THREE.Shape();
    for (let i = 0; i < points.length; i++) {
      point = points[i];
      if (i === 0) {
      ...
      if (i in crossings) {
        ...
        if (crossing.number === activeCrossing) {
          ...
          // 0=>1, 1=>0, 5=>4
          activeCrossing =
            crossing.number + 2 * ((crossing.number + 1) % 2) - 1;
  ...
  move(point) {
    ...
    const pointX =
      (point.x - center[0]) * Math.cos(angle) -
      (point.y - center[1]) * Math.sin(angle);
    const pointY =
      -(point.x - center[0]) * Math.sin(angle) -
      (point.y - center[1]) * Math.cos(angle);
    // was: intY =  (point.x - center[0]) * Math.sin(angle) + (point.y - center[1]) * Math.cos(angle);
  ...
  unMove(point) {
    ...
    const center = this.parameters.options.center;
    const pointX =
      point[0] * Math.cos(angle) + point[1] * Math.sin(angle) + center[0];
    const pointY =
      -1 * point[0] * Math.sin(angle) + point[1] * Math.cos(angle) - center[1];
    return [pointX, pointY];
  }
