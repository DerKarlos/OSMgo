...
Way.prototype.Building = function (material, roofMat) {
  ...
  if (part) {
    inc_buildingParts() /*err("Way Multipolygon way WITH building:part "+this.id)*/;
    if (partView < 0) return;
  }
  ...
  var partCount = 0;
  var partWay = 0;
  var min_height = 0; // https://www.openstreetmap.org/way/205501268
  var roofHeight = 0;
  ...
  for (var n in this.wayNodes) {
    ...
    // tttx if ((partCount > partMax || partCount > 6) && partWay == 1 && !part && partView > 0)  // partCount>10??
    // tttx 	return
    if (
  ...
  if ((tagLevels = this.GetTag("building:levels", false, true) * 1)) {
    height = tagLevels * levelHeight;
  } // Above ground, mostly positiv           1 *3 = +3m
  if ((tagLevels = this.GetTag(/*******/ "levels", false, true) * 1)) {
    height = tagLevels * levelHeight;
  } // not dokumented but used
  if ((tagLevels = this.GetTag("building:max_level", false, true) * 1)) {
    height = (tagLevels + 1) * levelHeight;
  } // Example: only ground floor =>  0 => (0+1)*3 = +3m height
  if ((tagLevelm = this.GetTag("building:min_level", false, true) * 1))
    min_height = tagLevelm * levelHeight; // may be negativ  one cellar => -1 =>   -1 *3 = -3m dept
  if ((tagLevelm = this.GetTag("min_level", false, true) * 1))
    min_height = tagLevelm * levelHeight; // may be negativ  one cellar => -1 =>   -1 *3 = -3m dept
  // absolut values overrool level couts
  if ((tagHeight = this.GetTag(/***********/ "height", false, true) * 1)) {
    height = tagHeight * 1;
  }
  if ((tagHeight = this.GetTag("building:height", false, true) * 1)) {
    height = tagHeight * 1;
  }
  ...
  if (gOptimize == 0 || d0) {
    ...
    if (roofMesh) {
      // mesh.add(roofMesh)
      var geometry = roofMesh.geometry.translate(
        roofMesh.position.x,
        roofMesh.position.y,
        roofMesh.position.z,
      );
...
function PlaceRoof(way, min_height, height, roofHeight, points, materialX) {
  // Render function /// bbb bbb bbb bbb bbb bbb bbb bbb bbb bbb bbb bbb bbb
  ...
  var roofShape;
  if (
    !(roofShape = way.GetTag("roof:shape")) &&
    !(roofShape = way.GetTag("building:roof:shape"))
  ) {
    //var small = Math.sqrt( shape.getLength() )  < 10
    //if(!small)
    if (points.length == 5 && materialX[0] == mlm.buildingRoof)
      roofShape = "gabled";
    else return 0;
    //roofShape = "gab led"
  }
  ...
  switch (roofShape) {
    ...
    case "dome": ///////////////////////////////////////////// ddd
      ...
      var p = 0;
      for (p in points) {
        // all points of a ring
        // calculate angel center to side-point
        var x = points[p].x - x0;
        var z = points[p].y - z0;
        for (var a = 0; a <= 90; a += 90 / c) {
          // all angles
          var sc = SinCos(g(a));
          vertices.push(x * sc[1], -roofHeight * (1 - sc[0]), z * sc[1]);
          v++;
          if (p > 0 && a > 0) {
    ...
    case "onion":
      ...
      var Kurve = [
        //  // |y|  -x-   The curve is about "taken" frim F4map.com
        [1.0, 0.0],
        [1.12, 0.09],
        [1.27, 0.15],
        [1.36, 0.27],
    ...
    case "skillion": //// Roof will replace the whole building!
      ...
      if (!across)
        // Ussually the slope gets down from that Side. BUT: If the rotation is different, the slope is different! Revert??
        b += g(90);
      ...
      for (var i = 0; i < vlen; i++) {
        ...
        if (y >= -0.001) {
          // It's the roof, not the lower floor of the building(block)
    ...
    case "gabled":
      ...
      // was: const center = BuildingShapeUtils.center(this.shape, angle / 180 * Math.PI);
...
function edgeDirection(points) {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (angle >= Math.PI) {
      ...
      // 1. Error in Code of Building-Viewer?!
 ...
 */
function calc_extents(points, angle = 0) {
  // TEST_ONLY
  var x = [];
  var y = [];
  var vec;
